    ...
    <aside>
            ...
            {item.children ? (
              <div>
                <button
                  ...
                  className="w-full flex items-center justify-between gap-2 py-[8px] text-left text-[#161616] hover:text-[#2563eb] transition-colors"
                  ...
                  <div className="pl-3 border-l-2 border-slate-100 ml-0.5 mb-1 space-y-0.5">
                    {item.children.map((child) => (
                      <Link
                        key={child.href}
                        href={child.href}
                        className="block py-1.5 text-sm text-slate-600 hover:text-[#2563eb]"
                      >
                        {child.label}
                      </Link>
                    ))}
              ...
              <a
                ...
                className="block py-[8px] text-[#161616] hover:text-[#2563eb] transition-colors"
              >
                {item.label}
              </a>
            ) : (
              <Link href={item.href} className="block py-[8px] text-[#161616] hover:text-[#2563eb] transition-colors">
                {item.label}
              </Link>
            )}
          </div>
      ...
      {WIDGETS.map((w) => (
        /* Valós CSS: .box { margin-bottom: 30px; background-color: #cce9eb; width: 100%; }
           .box h1 { padding: 8px 15px; font-size: 18px; font-weight: 700; color: #FFF; background: #2563eb; margin: 0; }
        ...
        >
          ...
          <a
            ...
            target="_blank"
            rel="noopener noreferrer"
            style={{ display: 'block', backgroundColor: '#2563eb', padding: '8px 15px' }}
          >
            <div
