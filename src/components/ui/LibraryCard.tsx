...
export const LibraryCard: React.FC<LibraryCardProps> = ({
  ...
  return (
    <div className="bg-white rounded-xl border border-slate-200 p-5 shadow-sm card-hover-effect flex flex-col justify-between">
      <div>
        <span className="text-xs px-2.5 py-1 rounded font-medium bg-amber-50 text-[#F3701D] border border-amber-200/60 inline-block mb-3">
          {typeLabels[type] || 'Tagkönyvtár'}
        </span>

        <h3 className="font-bold text-slate-900 text-lg mb-3 hover:text-[#F3701D] transition-colors">
          <Link href={linkHref}>{name}</Link>
        </h3>

        <div className="space-y-2 text-xs text-slate-600 mb-4">
          <div className="flex items-start gap-2">
            <MapPin className="w-4 h-4 text-[#DDB837] shrink-0 mt-0.5" />
            <span>{address}</span>
          </div>
      ...
      <div className="pt-3 border-t border-slate-100 flex items-center justify-between text-xs font-semibold text-[#F3701D]">
