...
export const EventCard: React.FC<EventCardProps> = ({
  ...
  return (
    <div className="bg-white rounded-xl border border-slate-200 p-5 shadow-sm card-hover-effect flex gap-4">
      {/* Date badge */}
      <div className="shrink-0 w-16 h-20 rounded-lg bg-[#8C1D11] text-white flex flex-col items-center justify-center shadow-md">
      ...
      <div className="flex flex-col flex-1 min-w-0">
        <div className="flex items-center gap-2 mb-1">
          <span className="text-xs px-2 py-0.5 rounded bg-slate-100 font-medium text-slate-700 flex items-center gap-1">
            <Users className="w-3 h-3 text-[#C85A32]" />
            {audienceLabels[targetAudience] || 'Esemény'}
          </span>
        </div>

        <h3 className="font-bold text-slate-900 text-base leading-snug line-clamp-2 hover:text-[#8C1D11] transition-colors mb-2">
        ...
        <div className="mt-auto flex items-center justify-between pt-2 border-t border-slate-100 text-xs font-semibold">
          <Link href={`/esemenyek/${slug}`} className="text-[#8C1D11] hover:underline flex items-center gap-1">
            <span>Részletek</span>
          ...
          {registrationUrl && (
            <a
              ...
              className="bg-[#C85A32] hover:bg-[#b04b26] text-white px-2.5 py-1 rounded text-[11px] transition-colors"
            >
              Regisztráció
            </a>
          )}
