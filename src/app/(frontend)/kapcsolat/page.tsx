      ...
      <div>
          ...
          <tbody>
            {KIEMELT.map((row, i) => (
              <tr key={i}>
                ...
                <td className={TD_STYLE} style={{ backgroundColor: rowBg(i) }}>
                  {row.email && <a href={`mailto:${row.email}`} className="text-[#137F85] hover:underline">{row.email}</a>}
                </td>
              </tr>
            ))}
          </tbody>
          ...
          <tbody>
            {KOZPONTI.map((row, i) => (
              <tr key={i}>
                ...
                <td className={TD_STYLE} style={{ backgroundColor: rowBg(i) }}>
                  {row.email.includes('@') ? (
                    <a href={`mailto:${row.email}`} className="text-[#137F85] hover:underline">{row.email}</a>
                  ) : row.email}
                </td>
              </tr>
            ))}
        ...
        <div className="text-center mb-8">
          <Link
            ...
            style={{ backgroundColor: 'var(--link-text-a11y)' }}
          >
            Munkatársaink
